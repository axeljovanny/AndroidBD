...
public final class DBOperations {
    ...
    public Cursor getEnglish(){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s",
                OrderDB.Tables.ENGLISH);
        return  database.rawQuery(sql, null);
    }

    public Cursor getEnglishById(String id){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s" +
                        " WHERE %s=?",
                OrderDB.Tables.ENGLISH,
                OrderContract.English.ID);
        String[] whereArgs = {id};
        return database.rawQuery(sql, whereArgs);
    }

    public String insertEnglish(English english){
        SQLiteDatabase database = db.getWritableDatabase();
        String idEnglish = OrderContract.English.generateIdEnglish();
        ContentValues values = new ContentValues();
        values.put(OrderContract.English.ID, idEnglish);
        values.put(OrderContract.English.VALUE, english.getValue());
        values.put(OrderContract.English.ABBREVIATION, english.getAbbreviation());
        values.put(OrderContract.English.SPELLING, english.getSpelling());
        database.insertOrThrow(OrderDB.Tables.ENGLISH, null, values);

        return idEnglish;
    }

    public boolean updateEnglish(English english){
        SQLiteDatabase database = db.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(OrderContract.English.VALUE, english.getValue());
        values.put(OrderContract.English.ABBREVIATION, english.getAbbreviation());
        values.put(OrderContract.English.SPELLING, english.getSpelling());
        String whereClause = String.format("%s=?", OrderContract.English.ID);
        String[] whereArgs = {english.getIdEnglish()};
        int result = database.update(OrderDB.Tables.ENGLISH,
                values, whereClause, whereArgs);
        return result>0;
    }

    public boolean deleteEnglish(String idEnglish){
        SQLiteDatabase database = db.getWritableDatabase();
        String whereClause = OrderContract.English.ID + "=?";
        String[] whereArgs = {idEnglish};
        int result =  database.delete(OrderDB.Tables.ENGLISH,
                whereClause, whereArgs);
        return result > 0;
    }
